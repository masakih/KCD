...
final class AirCorpsSupplyCommand: JSONCommand {
    ...
    override func execute() {
        ...
        store.async {
            ...
            guard let areaId = self.parameter["api_area_id"].int else { return }
            guard let rId = self.parameter["api_base_id"].int else { return }
            ...
            if let v = self.data["api_distance"].int { airBase.distance = v }
            
            guard let material = store.material() else { return }
            
            if let v = self.data["api_after_bauxite"].int { material.bauxite = v }
            if let v = self.data["api_after_fuel"].int { material.fuel = v }
        }
    }
}
