...
class HMPortCommand: HMCompositCommand, NSCopying {
	...
	override class func canExcuteAPI(api: String) -> Bool {
		return api == "/kcsapi/api_port/port"
	}
	...
	class func commands() -> [AnyObject] {
		return [HMMemberShipCommand(), HMMemberMaterialCommand(),
			HMMemberDeckCommand(), HMMemberBasicCommand(),
			HMMemberNDockCommand(),
